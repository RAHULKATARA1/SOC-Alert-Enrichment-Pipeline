...
class SlackNotifier:
    ...
    def __init__(self):
        ...
        if self.offline_mode:
            logger.info("SlackNotifier running in Console Output Mode (no webhook configured)")
        else:
            logger.info("SlackNotifier initialized with Webhook")

    def notify(self, alert: Alert, severity_label: str, score: float, 
               recommended_action: str, ioc_reports: list[ThreatIntelReport]):
        """
        Send notification for a single alert.
        """
        blocks = self._build_block_kit(alert, severity_label, score, recommended_action, ioc_reports)
            ...
            "attachments": [
                {
                    ...
                    "blocks": [{"type": "section", "text": {"type": "mrkdwn", "text": " "}}] # Just for the colored bar
                }
            ]
        }
    ...
    def _build_block_kit(self, alert: Alert, severity: str, score: float, 
                         action: str, ioc_reports: list[ThreatIntelReport]) -> list[dict]:
        ...
        if malicious_iocs:
            ...
            for r in malicious_iocs:
                ...
                malware = f" ({', '.join(r.malware_families[:2])})" if r.malware_families else ""
        ...
        if alert.mitre_tactic:
            blocks.append({
                "type": "section",
                "text": {"type": "mrkdwn", "text": f"*MITRE Tactic:*\n{alert.mitre_tactic}"}
            })

        # Raw Log Snippet (Truncated)
        raw_snip = alert.raw_log[:200] + "..." if len(alert.raw_log) > 200 else alert.raw_log
        blocks.append({
            "type": "section",
            "text": {"type": "mrkdwn", "text": f"*Raw Log Evidence:*\n```{raw_snip}```"}
        })
