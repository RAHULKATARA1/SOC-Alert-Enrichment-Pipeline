...
class SeverityScorer:
    ...
    def calculate_severity(self, alert: Alert, ioc_reports: list[ThreatIntelReport]) -> tuple[str, float, dict]:
        """
        ...
        if alert.severity_hint.lower() == "critical" and final_score < self.thresholds["CRITICAL"]:
        ...
        # Determine Label
        severity_label = "INFO"
        for label, threshold in sorted(self.thresholds.items(), key=lambda x: x[1], reverse=True):
            if final_score >= threshold:
                severity_label = label
                break
                
        logger.info(f"Alert {alert.alert_id} scored {final_score:.1f} -> {severity_label}")
