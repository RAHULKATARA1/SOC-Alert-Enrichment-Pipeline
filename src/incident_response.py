...
class IncidentResponseEngine:
    ...
    def execute_playbooks(
        self, alert: Alert, severity: str, ioc_reports: list[ThreatIntelReport]
    ) -> list[dict]:
        """
        ...
        if severity in ["CRITICAL", "HIGH"]:
            for report in malicious_iocs:
                if report.ioc_type == "ip":
                    actions.append(
                        self._action_block_ip(report.ioc_value, alert.alert_id)
                    )
                elif report.ioc_type == "domain":
                    actions.append(
                        self._action_sinkhole_domain(report.ioc_value, alert.alert_id)
                    )
        ...
        if severity == "CRITICAL":
            if alert.alert_type in [
                ...
                internal_ip = (
                    alert.src_ip if self._is_internal(alert.src_ip) else alert.dst_ip
                )
                if internal_ip:
                    actions.append(
                        self._action_quarantine_host(internal_ip, alert.alert_id)
                    )
    ...
    def _action_create_ticket(self, alert: Alert, severity: str) -> dict:
        ...
        action = {
            "action_id": f"ACT-TKT-{int(datetime.now().timestamp())}",
            "type": "create_ticket",
    ...
    def _action_block_ip(self, ip_address: str, alert_id: str) -> dict:
        ...
        action = {
            "action_id": f"ACT-BLK-{int(datetime.now().timestamp())}",
            "type": "firewall_block",
            "timestamp": datetime.now().isoformat(),
            "target": ip_address,
            "alert_id": alert_id,
    ...
    def _action_sinkhole_domain(self, domain: str, alert_id: str) -> dict:
        ...
        action = {
            "action_id": f"ACT-DNS-{int(datetime.now().timestamp())}",
            "type": "dns_sinkhole",
            "timestamp": datetime.now().isoformat(),
            "target": domain,
            "alert_id": alert_id,
    ...
    def _action_quarantine_host(self, host_ip: str, alert_id: str) -> dict:
        ...
        action = {
            "action_id": f"ACT-ISO-{int(datetime.now().timestamp())}",
